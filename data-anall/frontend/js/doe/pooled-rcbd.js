...
function populateSelects(headers) {
    const ids = ['col-year', 'col-treat', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-treat', ['treat', 'genotype', 'var']);
    guess('col-rep', ['rep', 'block']);
    guess('col-resp', ['yield', 'val', 'height']);
}
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/analyze_pooled_rcbd', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    document.getElementById('bartlett-box').innerHTML = `
        <div class="glass-card" style="padding:1rem;">
            <p>Chi-Sq: <b>${b.stat ? b.stat.toFixed(4) : '-'}</b>, P-value: <b>${b.p !== undefined ? b.p.toFixed(4) : '-'}</b></p>
            <p>Result: <b style="color:${b.result === 'Homogeneous' ? '#4ade80' : '#f87171'}">${b.result || '-'}</b></p>
        </div>
    `;
    ...
    ab.innerHTML = '';
    const order = ["Year (Y)", "Rep (within Y)", "Treatment (T)", "Year x Treatment (Y x T)", "Error", "Total"];
    ...
    ["Year (Y)", "Treatment (T)", "Year x Treatment"].forEach(k => {
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_pooled_rcbd', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Pooled_RCBD_Report.docx";

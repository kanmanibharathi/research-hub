...
function populateSelects(headers) {
    const ids = ['col-main', 'col-sub', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-main', ['main', 'a', 'factor a', 'irrigation', 'sowing']);
    guess('col-sub', ['sub', 'b', 'factor b', 'fert', 'dose']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('main_col', document.getElementById('col-main').value);
    fd.append('sub_col', document.getElementById('col-sub').value);
    ...
    try {
        const res = await fetch('/analyze_split_plot', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    const keys = ["Main Plot (A)", "Sub Plot (B)", "Interaction A x B"];
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_split_plot', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Split_Plot_Report.docx";

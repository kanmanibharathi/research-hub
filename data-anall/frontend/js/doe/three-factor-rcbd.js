...
function populateSelects(headers) {
    const ids = ['col-fact-a', 'col-fact-b', 'col-fact-c', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-fact-c', ['factor c', 'fert', 'c']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('fact_c', document.getElementById('col-fact-c').value);
    ...
    try {
        const res = await fetch('/analyze_three_factor_rcbd', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    ab.innerHTML = '';
    const order = [
        "Replication", "Factor A", "Factor B", "Factor C",
        "Interaction A x B", "Interaction A x C", "Interaction B x C",
    ...
    const keys = ["Factor A", "Factor B", "Factor C", "Interaction A x B", "Interaction A x C", "Interaction B x C", "Interaction A x B x C"];
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_three_factor_rcbd', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "ThreeFactor_RCBD_Report.docx";

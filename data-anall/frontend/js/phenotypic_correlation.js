document.addEventListener('DOMContentLoaded', () => {
    ...
    analyzeBtn.addEventListener('click', async () => {
        ...
        try {
            const resp = await fetch('http://localhost:8000/analyze_phenotypic_correlation', {
    ...
    function renderResults(res) {
        ...
        traits.forEach(t => {
            ...
            vBody.innerHTML += `<tr>
                ...
                <td><strong>${v.sigma2_p.toFixed(4)}</strong></td>
            </tr>`;
        });

        // 3. Interpretation
        const interpList = document.getElementById('interpretation-list');
        interpList.innerHTML = res.interpretations.map(text => `<p style="margin-bottom: 0.5rem; border-left: 3px solid #d63384; padding-left: 10px;">${text}</p>`).join('');
    }
    ...
    function setupDownloads(formData) {
        ...
        setupBtn('dl-doc-btn', 'report_phenotypic_correlation_doc', 'Phenotypic_Correlation_Report.docx');
        setupBtn('dl-excel-btn', 'report_phenotypic_correlation_excel', 'Phenotypic_Correlation_Output.xlsx');
    }

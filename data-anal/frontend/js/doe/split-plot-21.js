...
function populateSelects(headers) {
    const ids = ['col-main1', 'col-main2', 'col-sub', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-main1', ['main1', 'a', 'factor a']);
    guess('col-main2', ['main2', 'b', 'factor b']);
    guess('col-sub', ['sub', 'c', 'factor c']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('main_a', document.getElementById('col-main1').value);
    fd.append('main_b', document.getElementById('col-main2').value);
    fd.append('sub_c', document.getElementById('col-sub').value);
    ...
    try {
        const res = await fetch('/analyze_split_plot_21', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    ab.innerHTML = '';
    const order = [
        "Replication", "Factor A (Main)", "Factor B (Main)", "Interaction A x B", "Error (a)",
        "Factor C (Sub)", "Interaction A x C", "Interaction B x C", "Interaction A x B x C", "Error (b)",
        "Total"
    ];
    order.forEach(k => {
    ...
    const keys = ["Factor A", "Factor B", "Interaction A x B", "Factor C", "Interaction A x C", "Interaction B x C", "Interaction A x B x C"];
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_split_plot_21', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Split_Plot_21_Report.docx";

...
function populateSelects(headers) {
    const ids = ['col-main', 'col-sub1', 'col-sub2', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-sub1', ['sub1', 'b', 'factor b']);
    guess('col-sub2', ['sub2', 'c', 'factor c']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('sub_b', document.getElementById('col-sub1').value);
    fd.append('sub_c', document.getElementById('col-sub2').value);
    ...
    try {
        const res = await fetch('/analyze_split_plot_12', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    ab.innerHTML = '';
    const order = [
        "Replication", "Factor A (Main)", "Error (a)",
    ...
    const keys = ["Factor A", "Factor B", "Factor C", "Interaction B x C", "Interaction A x B", "Interaction A x C", "Interaction A x B x C"];
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_split_plot_12', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Split_Plot_12_Report.docx";

...
function populateSelects(headers) {
    const ids = ['col-year', 'col-a', 'col-b', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-a', ['factor a', 'main', 'var', 'gen']);
    guess('col-b', ['factor b', 'sub', 'treat', 'nitro']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('col_year', document.getElementById('col-year').value);
    fd.append('col_a', document.getElementById('col-a').value);
    fd.append('col_b', document.getElementById('col-b').value);
    fd.append('col_rep', document.getElementById('col-rep').value);
    fd.append('col_resp', document.getElementById('col-resp').value);
    fd.append('alpha', document.getElementById('alpha').value);
    fd.append('post_hoc', document.getElementById('post-hoc').value);

    try {
        const res = await fetch('/analyze_pooled_two_factor_rcbd', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    document.getElementById('bartlett-box').innerHTML = `
        <div class="glass-card" style="padding:1rem;">
            <p>F-Ratio (Max/Min MS): <b>${b.ratio ? b.ratio.toFixed(4) : '-'}</b></p>
            <p>Result: <b style="color:${b.result && b.result.includes('Hetero') ? '#f87171' : '#4ade80'}">${b.result || '-'}</b></p>
    ...
    ab.innerHTML = '';
    const order = ["Year (Y)", "Rep (within Year)", "Factor A", "Factor B",
    ...
    <div style="margin-bottom:1rem; text-align:right; font-weight:bold; color:#4ade80;">
        CV %: ${ph.stats['CV'].toFixed(2)}
    </div>
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:1rem; margin-bottom:2rem;">
    `;

    const makeCard = (title, stats) => {
        if (!stats) return '';
        return `
        <div class="glass-card" style="padding:1rem;">
    ...
    ["Year (Y)", "Factor A", "Factor B", "Factor A x B"].forEach(k => {
        html += makeCard(k, ph.stats[k]);
    });
    html += `</div>`;
    ...
    ["Year (Y)", "Factor A", "Factor B", "Factor A x B"].forEach(k => {
        if (ph[k]) c.innerHTML += createTable(`${k} Means`, ph[k]);
    });
}
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_pooled_two_factor_rcbd', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Pooled_Two_Factor_Report.docx";

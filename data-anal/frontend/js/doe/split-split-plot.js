...
function populateSelects(headers) {
    const ids = ['col-main', 'col-sub', 'col-subsub', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-subsub', ['sub-sub', 'c', 'factor c']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('sub_c', document.getElementById('col-subsub').value);
    ...
    try {
        const res = await fetch('/analyze_split_split_plot', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    ab.innerHTML = '';
    const order = [
        "Replication", "Main Plot (A)", "Error (a)",
    ...
    <div style="margin-bottom:1rem; text-align:right; font-weight:bold; color:#4ade80;">
        CV (a) %: ${ph.stats['CV (a)'].toFixed(2)} | CV (b) %: ${ph.stats['CV (b)'].toFixed(2)} | CV (c) %: ${ph.stats['CV (c)'].toFixed(2)}
    </div>
    ...
    const keys = ["Main Plot (A)", "Sub Plot (B)", "Interaction A x B", "Sub-Sub Plot (C)", "Interaction A x C", "Interaction B x C", "Interaction A x B x C"];
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_split_split_plot', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "Split_Split_Plot_Report.docx";

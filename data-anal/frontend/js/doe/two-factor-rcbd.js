...
function populateSelects(headers) {
    const ids = ['col-fact-a', 'col-fact-b', 'col-rep', 'col-resp'];
    ids.forEach(id => {
    ...
    guess('col-fact-a', ['factor a', 'treat', 'genotype', 'a']);
    guess('col-fact-b', ['factor b', 'bg', 'env', 'b']);
...
document.getElementById('analyze-btn').addEventListener('click', async () => {
    ...
    fd.append('fact_a', document.getElementById('col-fact-a').value);
    fd.append('fact_b', document.getElementById('col-fact-b').value);
    ...
    try {
        const res = await fetch('/analyze_two_factor_rcbd', { method: 'POST', body: fd });
...
function renderResults(data) {
    ...
    ab.innerHTML = '';
    const order = ["Replication", "Factor A", "Factor B", "Interaction A x B", "Error", "Total"];
    order.forEach(k => {
    ...
    // Overall Stats
    let html = `
    <div style="margin-bottom:1rem; text-align:right; font-weight:bold; color:#4ade80;">CV %: ${ph.stats.CV.toFixed(2)}</div>
    
    <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:1rem; margin-bottom:2rem;">
    `;

    // Helper to make card group
    ...
    html += makeCardGroup("Factor A", ph.stats['Factor A']);
    html += makeCardGroup("Factor B", ph.stats['Factor B']);
    html += makeCardGroup("Interaction A x B", ph.stats.Interaction);
    html += `</div>`;
    ...
    const createTable = (title, dataset) => {
        if (!dataset.means) {
            // Suppressed note
            return `<div class="glass-card" style="padding:1rem; text-align:center; color:#eda; margin-top:1rem;">
                ${title}: Not Significant. ${dataset.note || 'Comparisons suppressed.'}
            </div>`;
        }
    ...
    c.innerHTML += createTable("Factor A Means", ph['Factor A']);
    c.innerHTML += createTable("Factor B Means", ph['Factor B']);
    c.innerHTML += createTable("Interaction Means A x B", ph['Interaction']);
}
...
document.getElementById('download-btn').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/report_two_factor_rcbd', { method: 'POST', body: fd });
        if (res.ok) {
            const b = await res.blob();
            const url = window.URL.createObjectURL(b);
            const a = document.createElement('a');
            a.href = url;
            a.download = "TwoFactor_RCBD_Report.docx";

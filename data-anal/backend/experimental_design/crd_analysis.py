...
class CRDAnalyzer:
    ...
    def create_report(self):
        ...
        # Precision
        doc.add_paragraph(f"\nSE(m): {self.results['SEm']:.4f}")
        doc.add_paragraph(f"SE(d): {self.results['SEd']:.4f}")
        ...
        if p <= self.alpha:
            ...
            best_t = means.idxmax()
            best_val = means.max()
            doc.add_paragraph(f"Treatment {best_t} recorded the highest mean value of {best_val:.4f}.")
        else:

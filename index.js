...
function skewness(arr){
  ...
  const s = sd(v)
  if(s === 0) return 0
  const sumCubed = v.reduce((acc,x)=> acc + Math.pow((x - m)/s, 3), 0)
  // unbiased adjustment
  return (n / ((n-1)*(n-2))) * sumCubed
}

// Excess kurtosis (adjusted)
function kurtosis(arr){
  ...
  const s = sd(v)
  if(s === 0) return 0
  const sum4 = v.reduce((acc,x)=> acc + Math.pow((x - m)/s, 4), 0)
...
export default function Home(){
  ...
  const stats = useMemo(()=>{
    ...
    return columns.map(colName=>{
      ...
      const s = {
        ...
        skewness: Number.isFinite(skewness(numeric))? round(skewness(numeric),3): '',
        kurtosis: Number.isFinite(kurtosis(numeric))? round(kurtosis(numeric),3): '',
  ...
  function downloadResults(){
    ...
    const statsRows = stats.map(s=>({
      Variable:s.variable, Mean:s.mean, SD:s.sd, Maximum:s.maximum, Minimum:s.minimum, Median:s.median,
      CV:s.cv, Skewness:s.skewness, Kurtosis:s.kurtosis, PercentNA:s.pctna
    }))
